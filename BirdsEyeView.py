...
class BirdsEyeView(object):
    ...
    def computeBEVLookUpTable(self, cropping_ul = None, cropping_size = None):
        ...
        Z_mesh_vec = (np.reshape(XZ_mesh[1], np.int(self.bevParams.bev_size[0] * self.bevParams.bev_size[1]), order = 'F')).astype('f4')
        X_mesh_vec = (np.reshape(XZ_mesh[0], np.int(self.bevParams.bev_size[0] * self.bevParams.bev_size[1]), order = 'F')).astype('f4')
